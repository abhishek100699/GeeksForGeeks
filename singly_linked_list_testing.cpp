...
// delete fron the given position.
Node *delete_from_given_pos(Node *head , int pos){
    Node *curr = head;
    if(pos==0){
        return NULL;
    }
...
int main(){
   ...
   /* head = insert_at_given_pos(head , 3 , 25);
    ...
    display(head);
    cout << "The given data is present at the pos " << Search(head , 8) << endl;
    */
    cout << endl;
    cout << "After deleting from the given position " << endl;
    head = delete_from_given_pos(head , 3);
    display(head);
}
